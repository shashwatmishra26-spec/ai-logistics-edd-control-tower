"""
AI Intervention Simulator — transparent 85% -> 94% pathway.
...
def run():
    ...
    # --- Intervention 5: Early-risk escalation (systemic, residual gap) -----
    remaining_gap = max(0, round(denom * EDD_TARGET) - (baseline_met + running_recovered))
    recovered_5 = remaining_gap
    ...
    steps.append({
        ...
        "cumulative_edd_adherence": round((baseline_met + running_recovered) / denom, 4),
        "formula": f"Residual shipments needed to reach the {EDD_TARGET:.0%} target after interventions 1-4, "
    ...
    print(f"ACTUAL baseline: {baseline_rate:.2%} | PROJECTED after interventions 1-4: "
          f"{(baseline_met + running_recovered - recovered_5)/denom:.2%} | "
